...
printUsage(){
    ...
    echo "Usage: $0 QMGR_NAME AUTH_USER MQ_BIN_PATH"
    echo "Example $0 QM1 root /opt/mqm/bin"
}
...
# 2. Print out all available listener ports
getListenerPort(){
    available_ports=''
    listener_ports=$(echo "dis LISTENER(*) PORT" | runmqsc "$QMGR_NAME" |  grep -oP 'PORT\(\K\d+')
    if [ -n "$listener_ports" ]; then
        echo "$listener_ports" | while read -r port; do
            echo -e "\033[1;32mINFO: Test the listener port: $port. \033[0m"
        ...
        if nc -zv localhost "$LISTENER_PORT"; then
...
# 3. Check whether the channel with $CHANNEL_NAME exists
prepChannelAndTopic(){
    ...
    # 4. Verify the user authority of channel connection
    ...
    if [[ "$OUTPUT" == *"$EXPECTED_OUTPUT"* ]]; then
        ...
        # 5. Create a topic if it doesn't exist
...
# 6. Print out useful info you need in setting configuration.yaml
printConnInfo(){
    ...
    echo -e  "\033[1;31mIf you want to delete the resources you created, please make sure you have downloaded the remove-resources.sh script and run like following: \033[0m"
    echo "chmod +x remove-resources.sh"
    echo "./remove-resources.sh -listener $LISTENER_NAME -channel $CHANNEL_NAME -topic $TOPIC_NAME"
}
